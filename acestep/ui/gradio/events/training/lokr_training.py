...
def start_lokr_training(
    ...
    lokr_train_text_encoder: bool,
    lokr_train_dit: bool,
    lokr_use_fp8: bool,
    lokr_target_modules: str,
    lokr_full_matrix: bool,
    lokr_bypass_mode: bool,
    lokr_rs_lora: bool,
    lokr_unbalanced_factorization: bool,
    lokr_optimizer_type: str,
    learning_rate: float,
    train_epochs: int,
    ...
    try:
        ...
        if device_type == "cuda":
            # Windows: DataLoader multiprocessing is prone to deadlocks. Force 0 workers.
            is_windows = os.name == "nt"
            num_workers = 0 if is_windows else 4
            pin_memory = True
            prefetch_factor = 2
            persistent_workers = False if is_windows else True
            pin_memory_device, mixed_precision = "cuda", "bf16"
        ...
        # Parse target_modules from comma-separated string
        target_modules_list = [m.strip() for m in lokr_target_modules.split(",") if m.strip()]
        ...
        lokr_config = LoKRConfigClass(
            ...
            train_text_encoder=lokr_train_text_encoder,
            train_dit=lokr_train_dit,
            use_fp8=lokr_use_fp8,
            target_modules=target_modules_list,
            full_matrix=lokr_full_matrix,
            bypass_mode=lokr_bypass_mode,
            rs_lora=lokr_rs_lora,
        ...
        yield f"🚀 Initializing DiT trainer from {tensor_dir}...", "", initial_plot, training_state
        trainer = LoKRTrainer(
            dit_handler=dit_handler, lokr_config=lokr_config, training_config=training_config,
        )

        yield "🧠 Injecting LoKr adapters into decoder...", "", initial_plot, training_state
