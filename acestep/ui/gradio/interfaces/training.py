...
def create_training_section(dit_handler, llm_handler, init_params=None) -> dict:
    ...
    with gr.Tabs():
        ...
        with gr.Tab(t("training.tab_train_lora")):
            ...
            with gr.Row():
                optimizer_type = gr.Dropdown(
                    choices=["adamw", "adamw8bit", "sgd", "lion", "prodigy"],
                    value="adamw",
                    label="Optimizer",
                    info="Select optimizer type. AdamW is recommended for most cases. Prodigy is learning-rate free.", elem_classes=["has-info-container"],
                )
        ...
        with gr.Tab("🚀 Train LoKr"):
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    lokr_weight_decompose = gr.Checkbox(
                        ...
                        value=True,
                        info=t("training.lokr_weight_decompose_info"), elem_classes=["has-info-container"],
                    )
                    with gr.Row():
                        lokr_train_text_encoder = gr.Checkbox(
                            label=t("training.lokr_train_text_encoder"),
                            value=False,
                            info=t("training.lokr_train_text_encoder_info"), elem_classes=["has-info-container"],
                        )
                        lokr_train_dit = gr.Checkbox(
                            label=t("training.lokr_train_dit"),
                            value=True,
                            info=t("training.lokr_train_dit_info"), elem_classes=["has-info-container"],
                        )
                    lokr_use_fp8 = gr.Checkbox(
                        label="Use Pseudo FP8",
                        value=False,
                        info="Cast base model weights to FP8 to save VRAM (computation remains in bf16/fp16). Applies to both DiT and Text Encoder.", elem_classes=["has-info-container"],
                    )

                    gr.HTML("<h4>🔧 Advanced LoKr Options</h4>")

                    lokr_target_modules = gr.Textbox(
                        label="Target Modules",
                        value="q_proj,k_proj,v_proj,o_proj",
                        placeholder="q_proj,k_proj,v_proj,o_proj,gate_proj,up_proj,down_proj",
                        info="Comma-separated list of module names to apply LoKr to.", elem_classes=["has-info-container"],
                    )

                    with gr.Row():
                        lokr_full_matrix = gr.Checkbox(
                            label="Full Matrix",
                            value=False,
                            info="Use full matrix decomposition mode.", elem_classes=["has-info-container"],
                        )
                        lokr_bypass_mode = gr.Checkbox(
                            label="Bypass Mode",
                            value=False,
                            info="Enable bypass mode for alternative gradient flow.", elem_classes=["has-info-container"],
                        )

                    with gr.Row():
                        lokr_rs_lora = gr.Checkbox(
                            label="RS LoRA",
                            value=False,
                            info="Enable Rank-Stabilized LoRA mode.", elem_classes=["has-info-container"],
                        )
                        lokr_unbalanced_factorization = gr.Checkbox(
                            label="Unbalanced Factorization",
                            value=False,
                            info="Enable unbalanced factorization for asymmetric ranks.", elem_classes=["has-info-container"],
                        )
            ...
            with gr.Row():
                lokr_optimizer_type = gr.Dropdown(
                    choices=["adamw", "adamw8bit", "sgd", "lion", "prodigy"],
                    value="adamw",
                    label="Optimizer",
                    info="Select optimizer type. AdamW is recommended for most cases. Prodigy is learning-rate free.", elem_classes=["has-info-container"],
    ...
    return {
        ...
        "lokr_train_text_encoder": lokr_train_text_encoder,
        "lokr_train_dit": lokr_train_dit,
        "lokr_use_fp8": lokr_use_fp8,
        "lokr_target_modules": lokr_target_modules,
        "lokr_full_matrix": lokr_full_matrix,
        "lokr_bypass_mode": lokr_bypass_mode,
        "lokr_rs_lora": lokr_rs_lora,
        ...
        "lokr_optimizer_type": lokr_optimizer_type,
