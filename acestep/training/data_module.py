...
class PreprocessedTensorDataset(Dataset):
    ...
    def __init__(self, tensor_dir: str):
        ...
        total_paths = len(self.sample_paths)
        if total_paths > 100:
            logger.info(f"Validating {total_paths} tensor paths on disk...")
        
        self.valid_paths = []
        for i, p in enumerate(self.sample_paths):
            if i > 0 and i % 500 == 0:
                logger.info(f"  ... {i}/{total_paths} validated")
            
            # Use os.path.isfile for stricter validation than exists
            if os.path.isfile(p):
                self.valid_paths.append(p)
        
        missing_count = total_paths - len(self.valid_paths)
        if missing_count > 0:
            logger.warning(
                f"Some tensor files not found: {missing_count} missing"
            )
...
class PreprocessedDataModule(LightningDataModule if LIGHTNING_AVAILABLE else object):
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        # Lightning Fabric sets torch.set_default_device('cuda'), which causes
        # RandomSampler's internal generator (CPU) to clash with torch.randperm
        # expecting a CUDA device.  Fix: supply an explicit CPU generator.
