...
class LoRATrainer:
    ...
    def _train_with_fabric(
        ...
        # Optimizer selection based on training_config.optimizer
        ...
        if optimizer_name == "adamw8bit":
            if HAS_BNB and device_type == "cuda":
                ...
                logger.warning("bitsandbytes not available or not CUDA, falling back to standard AdamW")
                if device_type == "cuda":
                    optimizer_kwargs["fused"] = True
                optimizer = AdamW(trainable_params, **optimizer_kwargs)
        elif optimizer_name == "sgd":
            logger.info("Using SGD optimizer with momentum")
            optimizer = torch.optim.SGD(trainable_params, momentum=0.9, **optimizer_kwargs)
        elif optimizer_name == "lion":
            try:
                # Try to use Lion optimizer from external library if available
                from lion_pytorch import Lion
                logger.info("Using Lion optimizer")
                optimizer = Lion(trainable_params, **optimizer_kwargs)
            except ImportError:
            ...
            try:
                # Try to use Prodigy optimizer from external library if available
                from prodigyopt import Prodigy
                logger.info("Using Prodigy optimizer (learning-rate free)")
                # Prodigy doesn't need lr in kwargs, it computes its own
                prodigy_kwargs = {
                    "weight_decay": self.training_config.weight_decay,
                }
                optimizer = Prodigy(trainable_params, **prodigy_kwargs)
            except ImportError:
...
class DualModel(torch.nn.Module):
    def __init__(self, text_encoder: torch.nn.Module, decoder: torch.nn.Module):
        super().__init__()
        self.text_encoder = text_encoder
        self.decoder = decoder
    def forward(self, *args, **kwargs):
        # Container only, not called directly
        return None
...
class PreprocessedLoKRModule(nn.Module):
    ...
    def __init__(
        ...
        # Granular flags
        self.train_te = getattr(lokr_config, "train_text_encoder", False)
        self.train_dit = getattr(lokr_config, "train_dit", True)
        ...
        if check_lycoris_available():
            # Consolidation: PreprocessedLoKRModule is responsible for injection
            self.model, self.lycoris_net, self.lokr_info = inject_lokr_into_dit(model, lokr_config)
            
            # Explicitly move the model to the device after injection
            # This ensures all LyCORIS parameters ARE on the correct device
            self.model.to(self.device)
            if self.lycoris_net:
                self.lycoris_net.to(self.device)
            logger.info(f"LoKr injected (TE={self.train_te}, DiT={self.train_dit}): {self.lokr_info['trainable_params']:,} trainable params")
        else:
            self.model = model
            self.lokr_info = {}
    ...
    def training_step(
        ...
        with autocast_ctx:
            ...
            if self.train_te or (self.dit_handler is not None and getattr(self.lokr_config, "train_text_encoder_only", False)):
                ...
                if te_model is None:
                    # Fabric might have wrapped it
                    if hasattr(self.model, "_forward_module"):
                        te_model = getattr(self.model._forward_module, "text_encoder", self.model._forward_module)
                ...
                if embed_layer is None and hasattr(te_model, "_forward_module"):
                    embed_layer = getattr(te_model._forward_module, "embed_tokens", None)
                ...
                # model is already on self.device from __init__
            ...
            if decoder is None:
                # Try Fabric unwrapping if model is wrapped
                if hasattr(self.model, "_forward_module"):
                    decoder = getattr(self.model._forward_module, "decoder", self.model._forward_module)
            
            # If we still have a generation model instead of a transformer, drill down
            if decoder is not None and hasattr(decoder, "decoder") and not hasattr(decoder, "rotary_emb"):
                 # It has a .decoder but it is not a transformer itself (transformers usually have rotary_emb in this codebase)
            ...
            # Use checkpointing for decoder if training TE or if forced
...
class LoKRTrainer:
    ...
    def train_from_preprocessed(
        ...
        try:
            ...
            train_te = getattr(self.lokr_config, "train_text_encoder", False)
            train_dit = getattr(self.lokr_config, "train_dit", True)
            if getattr(self.lokr_config, "train_text_encoder_only", False):
                train_te = True
                train_dit = False

            # Select target model for injection
            if train_te and train_dit:
                # In V1, we use the DualModel container defined above
                # We MUST pass the actual transformer decoder (the DiT model), not the high-level generation model
                target_model = DualModel(self.dit_handler.text_encoder, self.dit_handler.model.decoder)
                # Injection will be handled by PreprocessedLoKRModule.__init__
            elif train_te:
                target_model = self.dit_handler.text_encoder
            else:
                target_model = self.dit_handler.model
            ...
            # --- Memory Optimization & Freezing ---
            # inject_lokr_into_dit already handled Pseudo FP8 and basic freezing.
            # We now ensure gradient checkpointing is enabled for both components.
            
            # Configure DiT (always configure even if frozen, to save memory during backward)
            decoder = getattr(self.module.model, "decoder", None)
            if decoder is None and hasattr(self.dit_handler.model, "decoder"):
                decoder = self.dit_handler.model.decoder
            
            if decoder is not None:
                ckpt_enabled, _, _ = _configure_training_memory_features(decoder)
                if train_dit:
                    self.module.force_input_grads_for_checkpointing = ckpt_enabled
                logger.info(f"DiT memory features: gradient_checkpointing={ckpt_enabled}")

            # Configure TE if training it
            if train_te:
                te_inner = getattr(self.module.model, "text_encoder", self.module.model if train_te and not train_dit else None)
                if te_inner is not None:
                    _configure_training_memory_features(te_inner)
                    logger.info("Text Encoder gradient checkpointing enabled")

            # Final check for input_grads_enabled (mostly for logging)
            _, _, input_grads_enabled = _configure_training_memory_features(
                decoder if decoder is not None else self.module.model
            )
            ...
            yield 0, 0.0, f"📂 Loaded {len(data_module.train_dataset)} preprocessed samples"
            yield 0, 0.0, f"🧠 Training components: TE={train_te}, DiT={train_dit}"
            if not input_grads_enabled and train_dit:
                yield 0, 0.0, "ℹ️ Input-grad hook not available; using explicit checkpointing fallback"
    ...
    def _train_with_fabric(
        ...
        if getattr(self.lokr_config, "train_text_encoder_only", False):
            target_inner_module = self.module.model
        else:
            target_inner_module = self.module.model.decoder

        if device_type == "mps" or precision.endswith("-mixed"):
            target_inner_module = target_inner_module.to(dtype=torch.float32)
        else:
            target_inner_module = target_inner_module.to(dtype=self.module.dtype)
        casted_trainable, total_trainable_tensors = _ensure_trainable_params_fp32(target_inner_module)
        ...
        # Optimizer selection based on training_config.optimizer
        optimizer_name = self.training_config.optimizer.lower()
        logger.info(f"LoKr using optimizer: {optimizer_name}")

        if optimizer_name == "adamw8bit":
            if HAS_BNB and device_type == "cuda":
                logger.info("LoKr using bitsandbytes 8-bit AdamW optimizer")
                optimizer = bnb.optim.AdamW8bit(trainable_params, **optimizer_kwargs)
            else:
                logger.warning("bitsandbytes not available or not CUDA, falling back to standard AdamW")
                if device_type == "cuda":
            ...
            logger.info("LoKr using SGD optimizer with momentum")
            optimizer = torch.optim.SGD(trainable_params, momentum=0.9, **optimizer_kwargs)
        elif optimizer_name == "lion":
            try:
                from lion_pytorch import Lion
                logger.info("LoKr using Lion optimizer")
                optimizer = Lion(trainable_params, **optimizer_kwargs)
            except ImportError:
                logger.warning("lion_pytorch not installed, falling back to AdamW")
                if device_type == "cuda":
            ...
            try:
                from prodigyopt import Prodigy
                logger.info("LoKr using Prodigy optimizer (learning-rate free)")
                prodigy_kwargs = {
                    "weight_decay": self.training_config.weight_decay,
                }
                optimizer = Prodigy(trainable_params, **prodigy_kwargs)
        ...
        target_inner_module, optimizer = self.fabric.setup(target_inner_module, optimizer)
        if getattr(self.lokr_config, "train_text_encoder_only", False):
            self.module.model = target_inner_module
        else:
            self.module.model.decoder = target_inner_module
        casted_opt_params, total_opt_params = _ensure_optimizer_params_fp32(optimizer)
        logger.info(
            f"Optimizer param dtype fixup: casted {casted_opt_params}/{total_opt_params} to fp32"
        )
        ...
        if resume_from and os.path.exists(resume_from):
            try:
                yield 0, 0.0, f"🔄 Loading LoKr checkpoint from {resume_from}..."
                ...
                if checkpoint_info["weights_path"]:
                    # Load weights into lycoris_net
                    load_lokr_weights(self.module.lycoris_net, checkpoint_info["weights_path"])

                    start_epoch = checkpoint_info["epoch"]
                    global_step = checkpoint_info["global_step"]

                    status_parts = [f"✅ LoKr resumed from epoch {start_epoch}, step {global_step}"]
                    if checkpoint_info["loaded_optimizer"]:
                        status_parts.append("optimizer ✓")
                    if checkpoint_info["loaded_scheduler"]:
                        status_parts.append("scheduler ✓")
                ...
                yield 0, 0.0, f"⚠️ Failed to load LoKr checkpoint: {e}, starting fresh"
                start_epoch = 0
                global_step = 0
        elif resume_from:
            yield 0, 0.0, f"⚠️ Checkpoint path not found: {resume_from}, starting fresh"
