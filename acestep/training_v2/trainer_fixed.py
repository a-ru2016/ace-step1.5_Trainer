...
class FixedLoRATrainer:
    ...
    def train(
        ...
        try:
            ...
            # Granular flags
            train_te = getattr(self.adapter_config, "train_text_encoder", False)
            train_dit = getattr(self.adapter_config, "train_dit", True)
            
            # Backward compatibility
            if getattr(self.adapter_config, "train_text_encoder_only", False):
                train_te = True
                train_dit = False
            ...
            if train_te and train_dit:
                ...
                if not hasattr(self.model, "text_encoder") or self.model.text_encoder is None:
                    from acestep.training_v2.model_loader import load_preprocessing_models
                    te_model, _ = load_preprocessing_models(cfg.model_name_or_path, device=device, dtype=dtype)
                    target_model = te_model
                else:
                    target_model = self.model.text_encoder
            else:
                target_model = self.model

            # Pseudo FP8 application (delegated to injection but trainer can do initial pass)
            if getattr(self.adapter_config, "use_fp8", False):
                from acestep.training.lokr_utils import apply_pseudo_fp8
                if train_te or train_dit:
                    # apply_pseudo_fp8 is safe to call multiple times, but let's be strategic.
                    # inject_lokr_into_dit will ALSO call it, but we can do it here for untuned components.
                    if hasattr(self.model, "decoder") and not train_dit:
                        apply_pseudo_fp8(self.model.decoder)
                    if hasattr(self.model, "text_encoder") and not train_te:
                        apply_pseudo_fp8(self.model.text_encoder)
                    logger.info("[VRAM] Applied Pseudo FP8 to frozen components in Trainer")
