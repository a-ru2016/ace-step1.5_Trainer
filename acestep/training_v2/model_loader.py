...
def load_preprocessing_models(
    ...
    text_path = None
    # Try multiple standard names
    for name in ["Qwen3-Embedding-0.6B", "qwen25_tokenizer", "text_encoder"]:
        candidate = ckpt / name
        if candidate.is_dir():
            text_path = candidate
            break
    ...
    if text_path is None:
        # Last resort: look for any directory with 'Embedding' in it
        for candidate in ckpt.iterdir():
            if candidate.is_dir() and "Embedding" in candidate.name:
                text_path = candidate
                break
    ...
    if text_path is not None:
        ...
        logger.warning("[WARN] Text encoder directory not found under %s", ckpt)
...
def load_text_encoder(
    ...
    if text_path is None:
        ...
        for p in Path(checkpoint_dir).iterdir():
            if p.is_dir() and "Embedding" in p.name:
                text_path = p
                break

    if text_path is None:
        raise FileNotFoundError(f"Text encoder directory not found under {checkpoint_dir}")
